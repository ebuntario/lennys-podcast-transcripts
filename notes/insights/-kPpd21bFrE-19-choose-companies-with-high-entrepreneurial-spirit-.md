---
type: insight
title: Choose companies with high entrepreneurial spirit for accelerated career growth
concepts:
  - "[[concepts/career-acceleration]]"
  - "[[concepts/entrepreneurial-culture]]"
  - "[[concepts/company-selection]]"
source_guest: Dmitry Zlokazov
source_episode: How Revolut trains world-class PMs: The "Local CEO" model, raw intellect & building wow products
source: "[[guests/dmitry-zlokazov|Dmitry Zlokazov]]"
---
For maximum career acceleration, seek companies with [[concepts/entrepreneurial-spirit|high entrepreneurial spirit]] that allow you to operate like a founder. These environments provide the steepest learning curves by giving significant ownership, exposure to complex problems, and opportunities to build from scratch—skills that translate directly to startup leadership.